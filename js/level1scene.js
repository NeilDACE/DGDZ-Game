class level1scene extends Phaser.Scene {
    ...
    create() {
        ...
        this.add.text(width / 2, 20, "Level 1: Chaos am Schreibtisch", { 
            fontSize: '28px', 
            fill: '#FFD700' 
        }).setOrigin(0.5);
        
        // 2. Kosmos Symbol (Ziel) verstecken
        this.kosmosSymbol = this.add.image(width / 2, height / 2, 'kosmos_symbol')
            .setScale(0.1)
            .setVisible(false)
        ...
        // 5. Feedback-Text
        this.feedbackText = this.add.text(width / 2, height - 30, `Ordnung: ${this.orderAchieved}/${this.chaosCount}`, { 
            fontSize: '24px', 
            fill: '#00FF00', 
            backgroundColor: '#1E1E1E'
        }).setOrigin(0.5).setDepth(10);
        
        this.showMessage("Bringe die Unordnung in Ordnung. Ziehe und klicke auf die chaotischen Objekte!", 4000);
    }
    ...
    createChaosDraggable(key, startX, startY, startRot, scale, targetX, targetY, targetRot) {
        ...
        item.on('pointerdown', (pointer) => {
            if (!item.getData('isOrdered') && pointer.downElement.tagName === 'CANVAS') {
                 ...
                 if (pointer.primaryDown) { // Stellt sicher, dass es der linke Mausklick ist
    ...
    checkOrder(item) {
        ...
        // 1. Position prüfen
        const posMatch = (Math.abs(item.x - tx) < POS_TOLERANCE) && 
    ...
    achieveOrder(item) {
        ...
        this.orderAchieved++;
        this.feedbackText.setText(`Ordnung: ${this.orderAchieved}/${this.chaosCount}`);
    ...
    levelComplete() {
        this.showMessage("Das Chaos ist gebannt! Das Symbol von Kosmos erscheint.", 5000);
